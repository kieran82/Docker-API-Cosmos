...
let contents = fs.readFileSync(
    current_path_location + "json/blockchain_config.json"
);
    ...
    try {
        ...
        if (!("folder_path" in DataIn)) {
            res
                .status(404)
                .send(
                    tools.responseFormat(null, "database prefix is missing", false, 404)
                );
        }
            ...
            res
                ...
                .send(
                    tools.responseFormat(
                        null,
                        "name is not found in config file",
                        false,
                        404
                    )
                );
        ...
        console.log(
            "nameobj is " +
            util.inspect(classData, {
                showHidden: false,
                depth: null
            })
        );
        DataIn.id = !("id" in DataIn) ?
            tools.objectTostring(DataIn, classData.keyName) :
            tools.numberTostring(DataIn.id);
        if (checkArray.includes(req.params.name)) {
            ...
            taskDao
                ...
                .then(() => {
                    taskDao
                        .getItem(DataIn.id, searchAddition)
                        .then(body => {
                            console.log(body);
                            console.log(
                                "Check Body " +
                            ...
                            console.log(
                                "" + classData.lower_name + " " + DataIn.id + " exists"
                            );
                            res.send(
                                tools.responseFormat(
                                    null,
            ...
            const result = await multi_network.keyExists(
                class_name,
                classData.methods.checkExists,
                DataIn.id
            );
            console.log("testing return results: " + result);
            console.log(
                "check " + classData.lower_name + " id " + DataIn.id + " exists"
            );
            if (tools.checkBool(result)) {
                ...
                if (result === true || result == "true") {
                    ...
                    res.send(
                        tools.responseFormat(
                            null,
                            DataIn.id + " " + classData.displayName + " does exists",
                            true,
                            200
                        )
                    );
                    ...
                    console.log(
                        "" + classData.lower_name + " id " + DataIn.id + " does not exists"
                    );
                    res
                        .status(404)
                        .send(
                            tools.responseFormat(
                                null,
                                DataIn.id + " " + classData.displayName + " does not exists",
                                false,
                                404
                            )
                        );
        ...
        res
            .status(404)
            .send(tools.responseFormat(null, "Error Occurred " + e, false, 404));
    }
};
...
const getitemroute = async (req, res) => {
    try {
        ...
        if (checkArray.includes(req.params.name)) {
            ...
            let configOBJ = tools.configFile(
                current_path_location,
                DataIn.folder_path
            );
            const multi_network = require(current_path_location + "network.js")(
                configOBJ
            );
            const result = await multi_network.readKeyValue(
                class_name,
                classData.methods.reading,
                DataIn.id
            );
            console.log("testing return results: " + result);
            console.log(
                "get " + classData.lower_name + " id " + DataIn.id + " details"
            );
            if (!tools.isEmpty(result)) {
                ...
                res.send(
                    tools.responseFormat(
                        result,
                        DataIn.id + " " + classData.displayName + " found",
                        true,
                        200
                    )
                );
                ...
                res
                    ...
                    .send(
                        tools.responseFormat(
                            null,
                            DataIn.id + " " + classData.displayName + " Not found",
                            false,
                            404
                        )
                    );
    ...
    try {
        ...
        if (checkArray.includes(req.params.name)) {
            ...
            taskDao
                ...
                .then(() => {
                    taskDao
                        .deleteItem(DataIn.id, searchAddition)
                        .then(body => {
                            console.log(body);
                            console.log(
                                "Delete Body " +
                            ...
                            if (!tools.isEmpty(body)) {
                                ...
                                console.log(
                                    "" + classData.lower_name + " " + DataIn.id + " deleted"
                                );
                                res.send(
                                    tools.responseFormat(
                                        body,
            ...
            const result = await multi_network.deleteKeyValue(
                class_name,
                classData.methods.reading,
                DataIn.id
            );
            console.log("testing return results: " + result);
            console.log(
                "deleting " + classData.lower_name + " id " + DataIn.id + " details"
            );
            if (tools.checkBool(result)) {
                console.log(
                    "" +
                ...
                res.send(
                    tools.responseFormat(
                        result,
                        DataIn.id + " " + classData.displayName + " successfully deleted",
                        true,
                        200
                    )
                );
                ...
                console.log(
                    ...
                    " id " +
                    DataIn.id +
                    " does not exists and can't be deleted"
                );
                res
                    .status(404)
                    .send(
...
const additemroute = async (req, res) => {
    try {
        ...
        if (checkArray.includes(req.params.name)) {
            ...
            const result = await multi_network.createKeyValue(
                class_name,
                classData.methods.creating,
                keychk,
                JSON.stringify(DataIn)
            );
            ...
            if (tools.checkBool(result)) {
                res.send(
                    tools.responseFormat(
                        result,
                        DataIn.id + " " + classData.displayName + " was added",
                        true,
                        200
                    )
                );
                ...
                res
                    ...
                    .send(
                        tools.responseFormat(
                            result,
                            DataIn.id + " " + classData.displayName + " was not added",
                            false,
                            404
                        )
                    );
...
const updateitemroute = async (req, res) => {
    try {
        ...
        if (checkArray.includes(req.params.name)) {
            ...
            const result = await multi_network.updateKeyValue(
                class_name,
                classData.methods.updating,
                keychk,
                JSON.stringify(DataIn)
            );
            console.log("get results: " + result);
            if (tools.checkBool(result)) {
                res.send(
                    tools.responseFormat(
                        result,
                        DataIn.id + " " + classData.displayName + " was updated",
                        true,
                        200
                    )
                );
                ...
                res
                    ...
                    .send(
                        tools.responseFormat(
                            null,
                            classData.displayName + " was not updated",
                            false,
                            404
                        )
                    );
...
const getHistoryroute = async (req, res) => {
    try {
        ...
        console.log(
            "getting " + classData.lower_name + " history " + DataIn.id + ""
        );
        const result = await multi_network.getHistoryForKey(
            class_name,
            classData.methods.get_history,
            DataIn.id
        );
        console.log("testing return results: " + result);
        console.log(
            "get " + classData.lower_name + " id " + DataIn.id + " history"
        );
        ...
        if (tools.isEmpty(result)) {
            console.error(
                ...
                " " +
                classData.displayName +
                " not exists or no history available"
            );
            res
                .status(404)
                .send(
            ...
            arr.forEach(function (element) {
                if ("Timestamp" in element) {
                    ...
                    if ("seconds" in TStamp) {
                        ...
                        element.formatted_datetime = dateFormat(
                            dt.toISOString(),
...
const getRangeroute = async (req, res) => {
    try {
        ...
        if (checkArray.includes(req.params.name)) {
            ...
            const result = await multi_network.getStateByRange(
                class_name,
                classData.methods.get_state_by_range,
                range_from,
                range_to
            );
            console.log("testing script results: " + result);
...
const getQueryroute = async (req, res) => {
    try {
        ...
        if (!("query_string" in DataIn)) {
            console.error(
                "" + classData.displayName + "  querystring does not exists"
            );
            res
                .status(404)
                .send(
                    tools.responseFormat(
                        null,
                        "The query string does not exists",
                        false,
                        404
                    )
                );
        ...
        if (checkArray.includes(req.params.name)) {
            ...
            console.log(
                "check " +
                classData.displayName +
                "  with queryString with query:" +
                DataIn.query_string +
                ""
            );
            console.log(
                "" + classData.displayName + " query results: " + DataIn.query_string
            );
            const result = await multi_network.getQueryResult(
                class_name,
                classData.methods.get_query_result,
                DataIn.query_string
            );
            ...
            if (tools.isEmpty(result)) {
                ...
                res
                    ...
                    .send(
                        tools.responseFormat(
                            null,
                            classData.displayName + " query returned no result",
                            false,
                            404
                        )
                    );
